...
<main class="h-full pb-16 overflow-y-auto">
  <div class="container max-w-4xl px-6 mx-auto py-8">
    ...
    <div class="detail-card p-6 mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <div class="space-y-3">
            <p><span class="font-medium">Nama:</span> {{ $ite->name }}</p>
            <p><span class="font-medium">No Telepon:</span> {{ $item->user->phone }}</p>
            <p><span class="font-medium">Tanggal:</span> {{ $ite->created_at->format('l, d F Y - H:i:s') }}</p>
        ...
        <div class="flex flex-col items-center">
          ...
          <img class="preview-image w-full max-w-xs" src="{{ Storage::url($item->image) }}" alt="Bukti Pengaduan" loading="lazy" />
        </div>
      </div>
      @endforeach
      ...
      <div class="mt-6">
        ...
        <form action="{{ route('pengaduan.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="mt-6">
          @csrf
          @method('PUT')
          ...
          <div class="mb-4">
            ...
            <textarea name="description" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" rows="4" placeholder="Edit Keterangan">{{ $item->description }}</textarea>
          </div>
    ...
    <div class="detail-card p-6 mb-8">
      ...
      <div class="response-card p-5 rounded-lg">
        <div class="flex items-start">
          ...
          <div class="ml-4">
            <p class="text-gray-700">{{ $tangap->tanggapan }}</p>
            <p class="text-sm text-muted mt-2">Ditanggapi pada: {{ $tangap->created_at->format('d M Y H:i') }}</p>
          </div>
        </div>
      </div>
      @endif
    ...
    <div class="text-center mt-8">
      <a href="{{ url()->previous() }}" class="inline-flex items-center btn-primary rounded-lg py-2 px-6 font-medium">
