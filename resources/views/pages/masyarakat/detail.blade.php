...
<main class="h-full pb-16 overflow-y-auto">
  <div class="container grid px-6 mx-auto">
    <div class="flex items-center justify-between my-6">
      ...
      <a href="{{ route('pengaduan.create') }}" class="flex items-center px-6 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-all duration-150">
              ...
              <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs card-hover">
                <div class="flex items-center mb-4">
                  <div class="relative mr-3 rounded-lg overflow-hidden" style="width: 120px; height: 90px;">
                    <img class="object-cover w-full h-full" src="{{ Storage::url($item->image) }}" alt="Bukti Pengaduan" loading="lazy" />
                  </div>
                  <div class="flex-1">
                    <h4 class="mb-1 font-semibold text-gray-600">Pengaduan #{{ $loop->iteration }}</h4>
                    <p class="text-xs text-gray-500">{{ $item->created_at->format('d M Y, H:i') }}</p>

                    <span class="status-badge inline-flex items-center mt-1 text-xs font-medium text-black rounded-full {{ $item->status == 'Belum di Proses' ? 'bg-red-100' : ($item->status == 'Sedang di Proses' ? 'bg-orange-100' : 'bg-green-100') }}">
                      {{ $item->status }}
                    </span>
                  </div>
                </div>
                ...
                <div class="flex justify-between mt-4">
                  <a href="{{ route('pengaduan.show', $item->id) }}" class="btn-detail flex items-center text-white rounded-md">
